...
def run_joern(filepath: str, verbose: int):
    ...
    # print(command)
    command = str(svd.joern_dir() / "joern-cli" / command)
    # print(command)
    print(os.popen(command).read())
    # if verbose > 2:
    #     svd.debug(command)
    # output=svd.subprocess_cmd(command, verbose=verbose)
    # print(output)
    # try:
    #     shutil.rmtree(  "/home" / "workspace" / filename.name)
    ...
    #         print(E)
    #     pass
run_joern("/opt/linved/storage/processed/bigvul/before/42054.c",3)
def run_joern_after(filepath: str, verbose: int):
    """Extract graph using most recent Joern."""
    script_file = svd.external_dir() / "get_func_graph.scala"
    filename = svd.external_dir() / filepath
    params = f"filename={filename}"
    # path="/opt/joern"
    # print("22222222222222222222")
    #sudo /opt/joern/joern-cli/joern --script
    # /opt/linved/storage/external/get_func_graph.scala --params='filename=/opt/linved/storage/processed/bigvul/before/42054.c'
...
def full_run_joern(filepath: str, verbose=0):
    ...
    try:
        ...
        if verbose > 0:
            svd.debug(f"Failed {filepath}: {E}")
        return None
def full_run_joern_after(filepath: str, verbose=0):
    """Run full Joern extraction and save output."""
    try:
        run_joern_after(filepath, verbose)
        nodes, edges = get_node_edges(filepath)
        return {"nodes": nodes, "edges": edges}
    except Exception as E:
        if verbose > 0:
